class BookCopiesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @book_copy.update(book_copy_params)
        format.html { redirect_to @book_copy, notice: "Book copy was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to book_copies_path, notice: "Book copy was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_book_copy
      @book_copy = BookCopy.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def book_copy_params
      params.expect(book_copy: [ :heritage_code, :status, :book_id ])
    end
end
